...
#         # ---------------- INPUT (MUST BE LAST INSIDE CONTAINER) ----------------
...
with tab_statecase:
    ...
    # ── STEP 1: Display full chat history (without input) ─────────
    if not st.session_state.chat_history:
        st.info(
            "👋 Ask me anything about company policies, procedures, or raise a support ticket.",
            icon="🤖"
    ...
    # ── STEP 2: Chat input (always at the bottom of the tab) ──────
    user_input = st.chat_input(
        placeholder="Ask anything about policies, procedures, or raise a ticket...",
        key="sc_chat_input"
    )
    ...
    if user_input:
        ...
        # Build context from everything before the new user message
        conversation_context = build_chat_context(st.session_state.chat_history[:-1])
